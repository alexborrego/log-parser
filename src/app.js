...
let inputLog = fs.readFileSync(args[0]).toString().split('\n');
...
/**
 * Converts JSON to CSV and writes it to a file.
 * @param {JSON} input 
 */
const createFile = (JSON) => {
    let csv = csvParser.parse(JSON);
...
.then((JSON) => createFile(JSON)).catch(() => console.log('Please provide a valid path to the log file.'));